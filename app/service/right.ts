...
export default class Rights extends Service {
  ...
  public async getRightsList(obj) {
    ...
    const { keyword, type, rightsType, level } = obj;
    if (keyword || type) {
      // 搜索
      const conditionList: any[] = [];
      if (keyword) {
        conditionList.push({ rightsName: { [Op.substring]: keyword } });
      }
      if (type) {
      ...
      return { list: rights, total: totalCount.count };
    } else if (rightsType || level) {
      // 获取上级分类
      const conditionList: any[] = [];
      if (rightsType) {
